...
def timer():
    return '''<section class="timer-panel" aria-label="Activity timer">
...
<div class="activity-title"><div><p class="eyebrow">{i:02} / 20 · {esc(a['kind'])}</p><h1>{esc(a['title'])}</h1><p>{esc(a['purpose'])}</p><div class="tags"><span>{esc(a['group'])}</span><span>{esc(a['prep'])}</span></div></div><button class="button secondary" id="present" aria-pressed="false">Put it on the board</button></div>
{timer()}
<div class="activity-layout"><section class="play-panel" aria-labelledby="play-title"><div class="play-top"><h2 id="play-title">On the board</h2><span id="round-count">Prompt 1</span></div><noscript><p>Turn on JavaScript to use the controls. You can also run this from the teacher instructions with paper and a timer.</p></noscript><div id="play"></div><p id="play-status" class="play-status" role="status"></p><div class="round-controls"><button class="button secondary" id="previous-round">← Previous prompt</button><button class="button primary" id="next-round">Next prompt →</button></div><div class="language-box"><h3>A few useful phrases</h3><p>{esc(a['frames'])}</p></div></section>
<aside class="teacher-panel" aria-labelledby="teacher-title"><h2 id="teacher-title">How to run it</h2><h3>Before you start</h3><p>{esc(a['setup'])}</p><h3>The 20 minutes</h3><ol class="lesson-steps">{steps}</ol>{note}<h3>The English they’re using</h3><p>{esc(a['skills'])}</p><details><summary>Make it easier or harder</summary><p><strong>Keep it simple:</strong> {esc(a['support'])}</p><p><strong>Take it further:</strong> {esc(a['stretch'])}</p></details></aside></div>
...
<section class="tutorial-timer"><h2>The timer</h2><p>Use 20 minutes for the whole session, or one minute for a short speaking turn. Changing the length resets and pauses the timer.</p>''' + timer() + '''</section>
...
<article><h2>Using the board</h2><p><strong>Put it on the board</strong> hides the teacher instructions and enlarges the task. <strong>Back to teacher notes</strong> brings them back.</p><p>For guessing and drawing games, turn the guesser or drawer away before showing a secret card. Hide it before they turn back. Enlarging the page doesn’t hide the answer.</p></article>
<article><h2>A couple of practical details</h2><p>The vote buttons are for you to enter a show of hands. Students don’t need their own devices. Ranking arrows move a shared list; groups can make their own lists on paper.</p><p>Changing the prompt clears its answers. Leaving or refreshing the page clears the timer as well. Nothing is saved or sent anywhere.</p></article></section>
